using CNTK;
using ReadFromBitCoin.CNTK.CSTrainingExamples;
using System;
...
{
    public partial class FrmCNTK : Form
    {
        public FrmCNTK()
        {
            InitializeComponent();
        ...
        public void PrintTrainingProgress(Trainer trainer, int minibatchIdx, int outputFrequencyInMinibatches)
        {
            if ((minibatchIdx % outputFrequencyInMinibatches) == 0 && trainer.PreviousMinibatchSampleCount() != 0)
            {
                ...
                richCNTK.Text += ($"Minibatch: {minibatchIdx} CrossEntropyLoss = {trainLossValue}, EvaluationCriterion = {evaluationValue}\r\n");
            }
        }
...
#pragma warning restore 618
        ...
        public void TrainAndEvaluate(DeviceDescriptor device)
        {
            // build a logistic regression model
            Variable featureVariable = Variable.InputVariable(new int[] { inputDim }, DataType.Float);
            Variable labelVariable = Variable.InputVariable(new int[] { numOutputClasses }, DataType.Float);
